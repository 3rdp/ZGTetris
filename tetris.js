...
function drawQueue() {
    ...
    if (top == 5) {
        topPiece.rotationIndex =
            (topPiece.rotationIndex + 1) % topPiece.tetromino.length;
        topPiece.currentRotation = topPiece.tetromino[topPiece.rotationIndex];
    }

    if (mid == 5) {
        midPiece.rotationIndex =
            (midPiece.rotationIndex + 1) % midPiece.tetromino.length;
        midPiece.currentRotation = midPiece.tetromino[midPiece.rotationIndex];
    }

    if (bot == 5) {
        botPiece.rotationIndex =
            (botPiece.rotationIndex + 1) % botPiece.tetromino.length;
...
Piece.prototype.lock = function () {
    ...
    document.getElementById("Combos-value").innerHTML = combos;
};
...
Piece.prototype.rotate = function () {
    let nextPattern = this.tetromino[
        (this.rotationIndex + 1) % this.tetromino.length
    ];
    let kick = 0;
